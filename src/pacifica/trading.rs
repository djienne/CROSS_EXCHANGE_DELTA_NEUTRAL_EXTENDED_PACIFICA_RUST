...
use super::types::{PacificaFundingRate, PacificaMarketInfo, PacificaPosition};
...
pub struct PacificaTrading {
    ...
    market_info_cache: Option<HashMap<String, PacificaMarketInfo>>,
}
...
impl PacificaTrading {
    ...
    ///
    /// # Returns
    /// Funding rate information for the symbol
    pub async fn get_funding_rate(&mut self, symbol: &str) -> Result<PacificaFundingRate> {
        let market_info = self.get_market_info().await?;

        let info = market_info
            .get(symbol)
            .context(format!("Symbol {} not found in market info", symbol))?;

        Ok(PacificaFundingRate::from_market_info(
            info.symbol.clone(),
            info.funding_rate.clone(),
            info.next_funding_rate.clone(),
        ))
    }
    ...
    ///
    /// # Returns
    /// Vector of funding rate information for all markets
    pub async fn get_all_funding_rates(&mut self) -> Result<Vec<PacificaFundingRate>> {
        let market_info = self.get_market_info().await?;

        let mut funding_rates: Vec<PacificaFundingRate> = market_info
            .values()
            .map(|info| {
                PacificaFundingRate::from_market_info(
                    info.symbol.clone(),
                    info.funding_rate.clone(),
                    info.next_funding_rate.clone(),
                )
        ...
        funding_rates.sort_by(|a, b| a.symbol.cmp(&b.symbol));

        info!("[PACIFICA] Fetched funding rates for {} markets", funding_rates.len());
    ...
    /// * `symbol` - Trading symbol (e.g., "SOL", "BTC")
    /// * `side` - Order side (Buy or Sell)
    /// * `size` - Order size in base asset (e.g., 0.1 BTC)
    /// * `slippage_percent` - Maximum acceptable slippage percentage (e.g., 0.5 for 0.5%)
    ...
    pub async fn place_market_order(
        ...
        info!(
            "[PACIFICA] Placing MARKET {} order: {} {} (slippage: {}%)",
            match side { OrderSide::Buy => "BUY", OrderSide::Sell => "SELL" },
            rounded_size,
            symbol,
            slippage_percent
        ...
        info!(
            "[PACIFICA] Market order placed successfully: ID={}, ClientID={}",
            order_id,
            client_order_id
        );
    ...
    pub async fn close_position(
        ...
        info!(
            "[PACIFICA] Closing {} position on {} (size: {})",
            position.side, position.symbol, position_size
        );
    ...
    ///
    /// # Returns
    /// Vector of open positions for the account
    pub async fn get_positions(&self) -> Result<Vec<PacificaPosition>> {
        let url = format!(
        ...
        if !response.status().is_success() {
            let error_text = response.text().await?;
            anyhow::bail!("Failed to fetch positions: {}", error_text);
        }

        #[derive(Deserialize)]
        struct PositionsResponse {
            data: Vec<PacificaPosition>,
        }

        let positions_response: PositionsResponse = response.json().await?;

        info!(
            "[PACIFICA] Retrieved {} open position(s)",
            positions_response.data.len()
        );
    ...
    pub async fn get_position(&self, symbol: &str) -> Result<Option<PacificaPosition>> {
    ...
    /// * `leverage` - New leverage value (integer, e.g., 1, 5, 10)
    ///
    /// # Returns
    /// Ok(()) if leverage was updated successfully
    pub async fn update_leverage(&self, symbol: &str, leverage: i32) -> Result<()> {
        info!("[PACIFICA] Updating leverage for {} to {}x", symbol, leverage);
        ...
        if !response.status().is_success() {
            let error_text = response.text().await?;
            anyhow::bail!("Failed to update leverage: {}", error_text);
        }
        ...
        if !leverage_response.success {
            let error_msg = leverage_response.error.unwrap_or_else(|| "Unknown error".to_string());
            anyhow::bail!("Leverage update failed: {}", error_msg);
        }

        info!("[PACIFICA] Successfully updated leverage for {} to {}x", symbol, leverage);
