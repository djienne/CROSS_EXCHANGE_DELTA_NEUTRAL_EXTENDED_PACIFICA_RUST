...
async fn main() -> Result<(), ConnectorError> {
    ...
    println!("=== Extended DEX ETH Market Order Test ===\n");
    ...
    println!("    This will:");
    println!("    1. BUY ${:.2} of ETH-USD", notional);
    println!("    2. Wait 5 seconds");
    println!("    3. SELL ${:.2} of ETH-USD", notional);
    println!();
    ...
    let positions = client.get_positions(Some("ETH-USD")).await?;

    let eth_position = positions.iter().find(|p| p.market == "ETH-USD");

    let sell_quantity = if let Some(pos) = eth_position {
        println!("Found ETH position:");
        println!("  Side: {:?}", pos.side);
        println!("  Size: {} ETH", pos.size);
        println!("  Value: ${}", pos.value);
        println!();
        pos.size.parse::<f64>().unwrap_or(0.0)
    } else {
        println!("No ETH position found, using notional-based quantity");
        let qty = notional / mid_price;
        println!("  Calculated quantity: {:.6} ETH\n", qty);
        qty
    };

    // Step 3: Place market SELL order to close position using exact quantity
    println!("--- Step 3: Placing Market SELL Order ---");
    println!("Selling exact position size: {:.6} ETH\n", sell_quantity);
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            eprintln!("You may have an open ETH position. Check your Extended account.");
            eprintln!("\nPossible issues:");
            eprintln!("  - Order size too small (minimum requirements not met)");
    ...
    println!("\nNote: Check your Extended account to verify order fills and confirm zero ETH position.");
