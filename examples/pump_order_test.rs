...
async fn main() -> Result<(), ConnectorError> {
    ...
    println!("=== Extended DEX PUMP Market Order Test ===\n");
    println!("Environment: {}", env_type);
    println!("Market: PUMP-USD");
    ...
    println!("Approximate Quantity: {:.2} PUMP", approx_qty);
    println!("(Minimum required: 1000 PUMP = ${:.2})\n", mid_price * 1000.0);

    // Safety confirmation
    println!("⚠️  WARNING: You are about to execute REAL trades on {}!", env_type.to_uppercase());
    println!("    This will:");
    println!("    1. BUY ${:.2} of PUMP-USD", notional);
    println!("    2. Wait 5 seconds");
    println!("    3. SELL ${:.2} of PUMP-USD", notional);
    println!();
    ...
    let positions = client.get_positions(Some("PUMP-USD")).await?;

    let pump_position = positions.iter().find(|p| p.market == "PUMP-USD");

    let sell_quantity = if let Some(pos) = pump_position {
        println!("Found PUMP position:");
        println!("  Side: {:?}", pos.side);
        println!("  Size: {} PUMP", pos.size);
        println!("  Value: ${}", pos.value);
        println!();
        pos.size.parse::<f64>().unwrap_or(0.0)
    } else {
        println!("No PUMP position found, using notional-based quantity");
        let qty = notional / mid_price;
        println!("  Calculated quantity: {:.2} PUMP\n", qty);
        qty
    };

    // Step 3: Place market SELL order to close position using exact quantity
    println!("--- Step 3: Placing Market SELL Order ---");
    println!("Selling exact position size: {:.2} PUMP\n", sell_quantity);
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            eprintln!("You may have an open PUMP position. Check your Extended account.");
            eprintln!("\nPossible issues:");
            eprintln!("  - Order size too small (minimum requirements not met)");
    ...
    println!("\nNote: Check your Extended account to verify order fills and confirm zero PUMP position.");
